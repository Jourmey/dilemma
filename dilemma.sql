...
(
    ...
    `status`      tinyint(2) NOT NULL DEFAULT '0' COMMENT '任务状态 0未处理 1获取信息',
...
(
    ...
    `format`      varchar(10) NOT NULL DEFAULT '' COMMENT '链接:dash-flv360',
    `container`   varchar(10) NOT NULL DEFAULT '' COMMENT '类型:mp4',
    `quality`     varchar(10) NOT NULL DEFAULT '' COMMENT '质量:流畅 360P',
    `size`        tinyint(8) NOT NULL DEFAULT '0' COMMENT '任务大小',
    `create_time` timestamp   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` timestamp   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`)
...
(
    ...
    `update_time`  timestamp    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='视频下载表';
